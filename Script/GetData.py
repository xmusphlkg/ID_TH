...
for link in soup.find_all('a', class_='e1'):
    ...
    disease_list.append((disease_name, historical_url))
...
def download_file(disease_name, file_data):
    """
...
def download_files(disease_name, history_url):
    """
    Download all files from a given historical URL using multiple processes.
    """
    file_urls = get_file_url(history_url)
    with Pool(30) as p:
        p.starmap(download_file, [(disease_name, file_data) for file_data in file_urls])

# Run the script
if __name__ == '__main__':
    for disease_name, history_url in disease_list:
        download_files(disease_name, history_url)
