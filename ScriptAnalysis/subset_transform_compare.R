...
run_one <- function(shortname,
  ...
  summarize_uncertainty(
    ...
    select(
      Shortname, Method, Transform, n_paths, bsts_niter, PrimaryStatus,
      Pr_RP, Pr_BP, RP_Q025, RP_Q975, RP_Width, BP_Q025, BP_Q975, BP_Width
    )
}

args <- commandArgs(trailingOnly = TRUE)
default_diseases <- c("Syphilis", "HCV", "Scrub Typhus", "Shigellosis", "Dengue fever", "Leptospirosis")
diseases <- if (length(args) >= 1 && nzchar(args[1])) strsplit(args[1], ",")[[1]] else default_diseases
n_paths <- if (length(args) >= 2) as.integer(args[2]) else 1000L
bsts_niter <- if (length(args) >= 3) as.integer(args[3]) else 1000L
out_file <- if (length(args) >= 4) args[4] else "./temp/subset_transform_compare.csv"
...
results <- mclapply(
  ...
  function(i) {
    tryCatch(
      run_one(
        shortname = tasks$Shortname[i],
        transform_method = tasks$Transform[i],
        n_paths = n_paths,
        bsts_niter = bsts_niter,
        seed = 20251209L + i
      ) |>
...
comparison_input <- results |>
  filter(is.na(Error)) |>
  select(Shortname, Transform, RP_Width, BP_Width, Pr_RP, Pr_BP, PrimaryStatus)

comparison_log <- comparison_input |>
  filter(Transform == "log") |>
  select(-Transform) |>
  rename_with(~ paste0(.x, "_log"), -Shortname)

comparison_sqrt <- comparison_input |>
  filter(Transform == "sqrt") |>
  select(-Transform) |>
  rename_with(~ paste0(.x, "_sqrt"), -Shortname)

comparison <- full_join(comparison_log, comparison_sqrt, by = "Shortname") |>
  mutate(
    Delta_RP_Width = RP_Width_sqrt - RP_Width_log,
    Delta_BP_Width = BP_Width_sqrt - BP_Width_log
  )

dir.create(dirname(out_file), showWarnings = FALSE, recursive = TRUE)
write_csv(results, out_file)
write_csv(comparison, sub("\\.csv$", "_comparison.csv", out_file))

print(results)
if (any(!is.na(results$Error))) {
  cat("\nErrors\n")
  print(results |> filter(!is.na(Error)) |> select(Shortname, Transform, Error))
}
cat("\nComparison summary\n")
print(comparison)
