...
panel_irr <- wrap_plots(irr_list, ncol = 1, guides = "collect") &
...
plot_radar <- function(i) {
  ...
  ggplot(pdata, aes(x = month, y = norm_value, colour = Period, fill = Period)) +
    ...
    labs(title = title_lab, subtitle = subtitle_text)
}
...
full_fig <- panel_irr /
  panel_radar +
  plot_layout(heights = c(1.3, 3.5)) +
  plot_annotation(
    title   = "Seasonal displacement detection after COVID-19 disruption",
    caption = "Panels A–D: monthly observed-to-expected incidence rate ratios (IRR) by transmission group. Values <1 (blue) = suppression; >1 (orange/yellow) = exceedance; * marks monthly IRR > 10.\nPanels E–AB: normalized seasonal profiles (radar charts) comparing pre-pandemic observed (brown), post-PHSM observed (red), and post-PHSM counterfactual (teal).\nTriangles = circular center-of-mass peak; circles = empirical max peak. Annotations: Δpre = phase shift vs. pre-pandemic; Δcf = phase shift vs. counterfactual. Grey = rainy season (May–Oct).",
    theme = theme(
      plot.title   = element_text(face = "bold", size = 13, hjust = 0),
      plot.caption = element_text(size = 8, colour = COL_MUTED, hjust = 0)
    )
  )

ggsave(file.path(out_dir, "fig4.pdf"),
       full_fig, width = 18, height = 26,
       device = cairo_pdf, family = "Times New Roman", limitsize = FALSE)

ggsave(file.path(out_dir, "fig4.png"),
       full_fig, width = 18, height = 26, dpi = 200, limitsize = FALSE)
